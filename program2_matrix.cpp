...
//This program needs input value to run: give it size of matrix: ie to run: program2_matrix.cpp 4

//Program is written in "plain C++", using no Classes or shorthand notation.
//With Classes, program code can be made to be simpler and compact and
//make program look like the math.

//Put other small operations into functions, so main program looks more simple
//and logical.
...
#include <cmath>
#include <cstring>
#include <time.h>   //otherwise random # gen doesn't work
...
{        
  ...
  for(int j = 0; j < n; j++) {
    // initialize right-side of linear equations, column is the RHS of the equations. In A*x=y, y is column here.
