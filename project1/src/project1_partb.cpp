...
using namespace std;  // so don't need to write std:: in front of commands like cout, cin

// object for output files, for input files use ifstream
ofstream ofile;  //allows to work with output files in compact way

// Define functions used ahead of the main program, why is this an inline function: inline fnc:
//inline function: the compiler takes the return statements 'return  1-.0*exp(-10.0*x)' and pasts them in the corect spots
//instead of calling the function every time.  this saves floating point operations. Used for short (i.e. 1 line funcitons).
...
int main(int argc, char *argv[]){
  ...
  //If in command-line has less than 1 argument, it writes out an error.
   ...
   //for comments over many lines, start with /* .type here  and end with */

    // Loop over powers of 10 to allow to rerun the calculations for different n values, n = # of mesh (aka integration) pts
    //So this way can right away perform same calculation with different # of mesh pts.
    //i.e. if i = 1, n = 10.  if i = 3, n=1000 etc.
